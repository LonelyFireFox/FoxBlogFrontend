...
class LoginForm extends React.Component {
  ...
  loginSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        // 表单登录时，若验证码长度小于4则不会验证，所以这里要手动验证一次
        ...
        this.props.dispatch({
          ...
          res == 1 ? message.success("登录成功!") : message.error("用户名或密码错误!")
        })
  ...
  render() {
    ...
    return (
      <div className={this.props.className}>
        <h3 className={styles.title}>登录界面</h3>
        <Form onSubmit={this.loginSubmit}>
          ...
          <PromptBox info={getFieldError('password')} width={calculateWidth(getFieldError('password'))}/>}>
            {getFieldDecorator('password', {
              ...
              <Input
                ...
                placeholder='密码'
               /* addonBefore={
                  <Icon type="lock" theme="filled" style={focusItem === 1 ? styles.focus : {}}/>
                }*/
              />,
            )}
          </Form.Item>
          ...
          <div className={styles.bottom}>
            <input className={styles.loginBtn} type="submit" value='登录'/>
            <span className={styles.registerBtn} onClick={this.register}>注册</span>
          </div>
        </Form>
